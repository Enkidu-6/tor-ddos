...
for i in "${!array[@]}"; do
   ...
   printf "iptables -t mangle -A PREROUTING -p tcp --syn --destination %s --destination-port %s -m hashlimit --hashlimit-name TOR-%s --hashlimit-mode srcip --hashlimit-srcmask 32 --hashlimit-above 30/hour --hashlimit-burst 2 --hashlimit-htable-expire 120000 -j DROP\n" "${array[i]}" "${array2[i]}" "${array[i]}" 
...
for i in "${!ARRAY[@]}"; do
   ...
   printf "ip6tables -t mangle -A PREROUTING -p tcp --syn --destination %s --destination-port %s -m hashlimit --hashlimit-name TOR6-%s --hashlimit-mode srcip --hashlimit-srcmask 128 --hashlimit-above 30/hour --hashlimit-burst 2 --hashlimit-htable-expire 120000 -j DROP\n" "${ARRAY[i]}" "${ARRAY2[i]}" "${ARRAY3[i]}"
