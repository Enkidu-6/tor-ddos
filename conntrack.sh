...
echo -e "   \033[0;35m`perl -ne 'print if ($seen{$_} .= @ARGV) =~ /10$/'  6 file2 | wc -l`\033[0m Relays out of \033[0;35m`cat 6 | wc -l` \033[0m"IPs with two connections
echo -e "\033[0;32mTwo connections, dual-or:\033[0m"
echo -e "   \033[0;35m`perl -ne 'print if ($seen{$_} .= @ARGV) =~ /10$/'  6 dual-or | wc -l`\033[0m Dual-OR Relays out of \033[0;35m`cat 6 | wc -l` \033[0m"IPs with two connections
echo -e "\033[0;32mOne connection, Relays:\033[0m"
cat 5 | grep ' 1 ' | awk '{ print $2 }' > 7
echo -e "   \033[0;35m`perl -ne 'print if ($seen{$_} .= @ARGV) =~ /10$/'  7 file2 | wc -l`\033[0m Relays out of \033[0;35m`cat 7 | wc -l` \033[0m"IPs with one connection
echo -e "\033[0;32mOne connection, dual-or:\033[0m"
echo -e "   \033[0;35m`perl -ne 'print if ($seen{$_} .= @ARGV) =~ /10$/'  7 dual-or | wc -l`\033[0m Dual-OR Relays out of \033[0;35m`cat 7 | wc -l` \033[0m"IPs with one connection
/bin/rm -r 5 6 7
